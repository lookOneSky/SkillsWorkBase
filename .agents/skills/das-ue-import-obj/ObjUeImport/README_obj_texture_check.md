...
度量一个 OBJ 目录里每个模型的**纹理密度**，按超采样倍率分档，把所有模型目录整体拷到 `x1`、`x4`、`x16` 等桶，并输出一份 JSON 统计。`x1` 是已达标、无需缩图的数据，因而输出仍是一套完整数据。

不带下列参数启动（例如双击）会打开图形界面；带参数则是纯命令行模式，不弹窗，便于批处理。

图形界面扫描完成后，可右键任意数据行并选择“打开瓦块所在位置”，直接打开对应的原始瓦块目录。
...
- **面积**：解析 OBJ 的 `v` 与 `f`，多边形按扇形三角化，累加每个三角形的面积。长度统一换算到厘米，OBJ 坐标本身代表什么单位由 `--obj-unit` 声明（默认 `m`）。
...
桶名 = x{step^level}                            level 0 = x1（达标，仍拷贝）
...
| `[目标密度px/cm]` | `10`（1 米贴 1000 像素） |

| 选项 | 默认值 | 说明 |
| --- | --- | --- |
| `--obj-unit <单位>` | `m` | OBJ 坐标单位，可选 `mm cm dm m km in ft yd mi` |
...
```powershell
:: 全部用默认值：目标 10 px/cm，输出到 D:\Obj_texture
...
输出根目录/
├─ x1/                     # 已达标，贴图保持原样
│  └─ Tile_-001_-001/      # 原样保留相对目录结构，obj+mtl+纹理整体拷贝
...
**重跑安全**：每次运行会先清掉这些模型在所有桶里的旧副本再拷贝，因此改目标密度后模型换桶不会留下重复数据；本次用不到的空桶目录会被删除。达标模型统一拷到 `x1`，所有桶合起来就是完整输入数据。
...
| `objects[].copiedTo` | 实际拷到的位置；仅 `--no-copy` 模式下为空 |
...
不带参数启动即打开界面，七项参数与命令行一一对应（「划分档位」列出的是这次会生成的桶阶梯，换「档位步长」会跟着重排，上限尽量停在原来的像素倍数上），另有四个操作按钮：
...
- **一键处理**：先按当前设置分类拷贝，再调用同一发布目录里的 `image_resize.exe`，递归把 `x4` 桶纹理限制到最大 4096、`x16` 桶限制到最大 2048；最后在输出根目录生成唯一的 `AllAfterSmall`，汇总 `x1`、处理后的 `x4` 和 `x16`。汇总会去掉桶层级并保留每个模型的相对目录，原桶和中间目录保留。桶不存在或没有 JPG/PNG 时自动跳过；相同相对路径且内容相同的文件会去重，内容不同则报错，避免静默覆盖。
