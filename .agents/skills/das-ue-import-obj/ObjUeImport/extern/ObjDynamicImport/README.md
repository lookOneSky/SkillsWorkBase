...
`import_obj.json` 只保存导入行为；项目与 OBJ 路径由 `obj_ue_import.exe` 参数提供：
...
使用默认配置导入单个 OBJ 或整个目录：

```powershell
obj_ue_import.exe --ue-import "D:\data\model.obj" "D:\Project\MyProject.uproject"
obj_ue_import.exe --ue-import "D:\data\tiles" "D:\Project\MyProject.uproject"
obj_ue_import.exe --ue-import "D:\data\model.obj" "D:\Project\MyProject.uproject" `
    --import-config "D:\config\import_obj.json"
```

## 交互式 UE 实例启动与缓存

仓库同时提供独立 Qt 程序 `das_ue_launcher.exe`，用于获取或启动普通交互式 Unreal Editor。
它与 `obj_ue_import.exe` 共用 EngineAssociation、Editor target、插件模块清单、DLL 与 BuildId
检查逻辑，但分别选择交互式 EXE 和对应的 `*-Cmd.exe`，不会把 commandlet 当作可复用实例。
...
`mapObjImport_YYYYMMDD_HHMMSS` 由它接着调 `build_level.py` 建，两者都落在同一个
`DasDataInfo` 目录里。
