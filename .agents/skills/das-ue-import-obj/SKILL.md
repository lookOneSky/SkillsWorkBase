...
6. 报告本次批次目录（默认 `/Game/ObjImport/<YYYYMMDD_HHMMSS>`）、导入数量、汇总关卡（默认 `/Game/ObjImport/<YYYYMMDD_HHMMSS>/DasDataInfo/mapObjImport_<YYYYMMDD_HHMMSS>`）和日志路径。失败时保留原始错误，不要改工程资产或配置重试。
